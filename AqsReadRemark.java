package com.viomi.external.order.server;
...
/**
 * @author lanmengxi@viomi.com
 * @createtime 2020/9/3 15:52
 * @description aqs源码走读备注，可以参考https://www.cnblogs.com/fnlingnzb-learner/p/13534174.html和https://www.jianshu.com/p/7e3f181f5823
 * @since version 初始于版本1.0.0
 */
public class AqsReadRemark extends AbstractOwnableSynchronizer implements java.io.Serializable
{
    
    private static final long serialVersionUID = 7373984972572414691L;
    
    protected AqsReadRemark()
    {
    }
    ...
    static final class Node
    {
        ...
        // Used to establish initial head or SHARED marker:建立初始化的头部节点，或者共享模式下使用
        Node()
        {
        }
     ...
     * method setHead. Note: If head exists, its waitStatus is guaranteed not to be CANCELLED.
     */
     ...
     * @return {@code true} if successful. False return indicates that the actual value was not equal to
     *         the expected value.
     */
     ...
     * estimate suffices to improve responsiveness with very short timeouts.
     */
    static final long spinForTimeoutThreshold = 1000L;
    
    /**
     * Inserts node into queue, initializing if necessary. See picture above. 将节点插入队列，如果队列还未初始化，则先进行初始化
     * 
    ...
    {
        // 通过自旋方式，一直尝试从队尾插入直到成功（对比为什么使用while(true)?）
        for(;;)
        {
            Node t = tail;
            // 队列未初始化
    ...
    /**
     * Creates and enqueues node for current thread and given mode. 添加当前线程的等待节点到同步队列的尾部
     * 
    ...
    private Node addWaiter(Node mode)
    {
        ...
        // Try the fast path of enq; backup to full enq on failure 尝试在尾部快速添加
        // 原尾部节点
        Node pred = tail;
        if(pred != null)
        {
            // 先将需要插入节点的前区节点指向原尾部节点
            node.prev = pred;
            // 通过cas，尝试将tail设置成当前节点
            if(compareAndSetTail(pred, node))
            {
                // 成功后原尾部节点的后驱节点设置成当前节点
                pred.next = node;
                return node;
            }
        }
        // 通过上述快速添加的方式插入失败，则尝试通过自旋方式插入节点
        enq(node);
        return node;
    }
    ...
    /**
     * Wakes up node's successor, if one exists. 唤醒指定节点的继任者节点里面的线程
     * 
     * @param node the node
     */
    private void unparkSuccessor(Node node)
    {
         ...
         * It is OK if this fails or if status is changed by waiting thread.当前节点的状态，如果是负数，一般是-1（待唤醒继任者）状态，
         * 则尝试清除节点状态（设置成0）
         */
         ...
         * 唤醒继任者里的线程，如果继任者状态被取消，或者是空，则从队列的尾部遍历获取没有被取消最前面的继任者
         */
        Node s = node.next;
    ...
    {
        /*
         * Ensure that a release propagates, even if there are other in-progress acquires/releases. This
         * proceeds in the usual way of trying to unparkSuccessor of head if it needs signal. But if it does
         * not, status is set to PROPAGATE to ensure that upon release, propagation continues. Additionally,
         * we must loop in case a new node is added while we are doing this. Also, unlike other uses of
         * unparkSuccessor, we need to know if CAS to reset status fails, if so rechecking.
         */
        for(;;)
        {
            // 头节点
    ...
    /**
     * Sets head of queue, and checks if successor may be waiting in shared mode, if so propagating if
     * either propagate > 0 or PROPAGATE status was set.
     *
     * @param node the node
    ...
    private void setHeadAndPropagate(Node node, int propagate)
    {
         ...
         * h.waitStatus either before or after setHead) by a previous operation (note: this uses sign-check
         * of waitStatus because PROPAGATE status may transition to SIGNAL.) and The next node is waiting in
         * shared mode, or we don't know, because it appears null
         *
         * The conservatism in both of these checks may cause unnecessary wake-ups, but only when there are
         * multiple racing acquires/releases, so most need signals now or soon anyway.
         */
    ...
    /**
     * Cancels an ongoing attempt to acquire.取消正在尝试获
     *
     * @param node the node
     */
     ...
     * same, but annoyingly different. Only a little bit of factoring is possible due to interactions of
     * exception mechanics (including ensuring that we cancel if tryAcquire throws exception) and other
     * control, at least not without hurting performance too much.
     */
    ...
    /**
     * Acquires in shared interruptible mode. 共享模式下获取中断
     * 
     * @param arg the acquire argument
     */
     ...
     *
     * <p>
     * This method is always invoked by the thread performing acquire. If this method reports failure,
     * the acquire method may queue the thread, if it is not already queued, until it is signalled by a
     * release from some other thread. This can be used to implement method {@link Lock#tryLock()}.
     *
     * <p>
     * The default implementation throws {@link UnsupportedOperationException}.
     *
     * @param arg the acquire argument. This value is always the one passed to an acquire method, or is
     *            the value saved on entry to a condition wait. The value is otherwise uninterpreted and
     ...
     * The default implementation throws {@link UnsupportedOperationException}.
     *
     * @param arg the release argument. This value is always the one passed to a release method, or the
     *            current state value upon entry to a condition wait. The value is otherwise
     *            uninterpreted and can represent anything you like.
     * @return {@code true} if this object is now in a fully released state, so that any waiting threads
     *         may attempt to acquire; and {@code false} otherwise.
    ...
    /**
     * Attempts to acquire in shared mode. This method should query if the state of the object permits
     * it to be acquired in the shared mode, and if so to acquire it.
     *
     * <p>
     * This method is always invoked by the thread performing acquire. If this method reports failure,
     * the acquire method may queue the thread, if it is not already queued, until it is signalled by a
     * release from some other thread.
     *
     * <p>
     * The default implementation throws {@link UnsupportedOperationException}.
     *
     ...
     *         subsequent shared-mode acquire can succeed; and a positive value if acquisition in shared
     *         mode succeeded and subsequent shared-mode acquires might also succeed, in which case a
     *         subsequent waiting thread must check availability. (Support for three different return
     *         values enables this method to be used in contexts where acquires only sometimes act
    ...
    /**
     * Returns {@code true} if synchronization is held exclusively with respect to the current (calling)
     * thread. This method is invoked upon each call to a non-waiting {@link ConditionObject} method.
     * (Waiting methods instead invoke {@link #release}.)
     *
     * <p>
     * The default implementation throws {@link UnsupportedOperationException}. This method is invoked
     * internally only within {@link ConditionObject} methods, so need not be defined if conditions are
     * not used.
     *
     ...
     * thread is queued, possibly repeatedly blocking and unblocking, invoking {@link #tryAcquire} until
     * success or the thread is interrupted. This method can be used to implement method
    ...
    /**
     * Attempts to acquire in exclusive mode, aborting if interrupted, and failing if the given timeout
     * elapses. Implemented by first checking interrupt status, then invoking at least once
     * {@link #tryAcquire}, returning on success. Otherwise, the thread is queued, possibly repeatedly
     * blocking and unblocking, invoking {@link #tryAcquire} until success or the thread is interrupted
    ...
    /**
     * Releases in exclusive mode. Implemented by unblocking one or more threads if {@link #tryRelease}
     * returns true. This method can be used to implement method {@link Lock#unlock}.
     *
     * @param arg the release argument. This value is conveyed to {@link #tryRelease} but is otherwise
     *            uninterpreted and can represent anything you like.
     ...
     * interrupts and timeouts may occur at any time, a {@code true} return does not guarantee that any
    ...
    {
        /*
         * The first node is normally head.next. Try to get its thread field, ensuring consistent reads: If
         * thread field is nulled out or s.prev is no longer head, then some other thread(s) concurrently
         * performed setHead in between some of our reads. We try this twice before resorting to traversal.
         */
        Node h, s;
        Thread st;
        ...
        /*
         * Head's next field might not have been set yet, or may have been unset after setHead. So we must
         * check to see if tail is actually first node. If not, we continue on, safely traversing from tail
    ...
    /**
     * Returns {@code true} if the apparent first queued thread, if one exists, is waiting in exclusive
     * mode. If this method returns {@code true}, and the current thread is attempting to acquire in
     * shared mode (that is, this method is invoked from {@link #tryAcquireShared}) then it is
     * guaranteed that the current thread is not the first queued thread. Used only as a heuristic in
     ...
     * <a href="MyAqs#barging">barging</a>. Such a synchronizer's {@link #tryAcquire} method should
     * return {@code false}, and its {@link #tryAcquireShared} method should return a negative value, if
     * this method returns {@code true} (unless this is a reentrant acquire). For example, the {@code
     ...
     * structures. This method is designed for use in monitoring system state, not for synchronization
     * control.
     *
    ...
    /**
     * Returns a collection containing threads that may be waiting to acquire. Because the actual set of
     * threads may change dynamically while constructing this result, the returned collection is only a
     * best-effort estimate. The elements of the returned collection are in no particular order. This
     * method is designed to facilitate construction of subclasses that provide more extensive
    ...
    /**
     * Returns a collection containing threads that may be waiting to acquire in shared mode. This has
     * the same properties as {@link #getQueuedThreads} except that it only returns those threads
    ...
    /**
     * Returns true if a node, always one that was initially placed on a condition queue, is now waiting
     * to reacquire on sync queue.
     * 
     * @param node the node
    ...
    final boolean isOnSyncQueue(Node node)
    {
        ...
        if(node.next != null) // If has successor, it must be on queue
            return true;
        /*
         * node.prev can be non-null, but not yet on queue because the CAS to place it on queue can fail. So
         * we have to traverse from tail to make sure it actually made it. It will always be near the tail
         * in calls to this method, and unless the CAS failed (which is unlikely), it will be there, so we
         * hardly ever traverse much.
         */
        return findNodeFromTail(node);
    }
    
    /**
     * Returns true if node is on sync queue by searching backwards from tail. Called only when needed
     * by isOnSyncQueue.
     * 
    ...
    final boolean transferForSignal(Node node)
    {
         ...
         * waiting. If cancelled or attempt to set waitStatus fails, wake up to resync (in which case the
    ...
    /**
     * Transfers node, if necessary, to sync queue after a cancelled wait. Returns true if thread was
     * cancelled before being signalled.
     *
     * @param node the node
    ...
    final boolean transferAfterCancelledWait(Node node)
    {
        ...
        /*
         * If we lost out to a signal(), then we can't proceed until it finishes its enq(). Cancelling
         * during an incomplete transfer is both rare and transient, so just spin.
         */
    ...
    /**
     * Invokes release with current state value; returns saved state. Cancels node and throws exception
     * on failure.
     * 
     ...
     * Note that because timeouts and interrupts may occur at any time, a {@code true} return does not
     * guarantee that a future {@code signal} will awaken any threads. This method is designed primarily
    ...
    /**
     * Returns an estimate of the number of threads waiting on the given condition associated with this
     * synchronizer. Note that because timeouts and interrupts may occur at any time, the estimate
     * serves only as an upper bound on the actual number of waiters. This method is designed for use in
     ...
     * associated with this synchronizer. Because the actual set of threads may change dynamically while
     * constructing this result, the returned collection is only a best-effort estimate. The elements of
     * the returned collection are in no particular order.
     *
     * @param condition the condition
     * @return the collection of threads
     * @throws IllegalMonitorStateException if exclusive synchronization is not held
    ...
    /**
     * Condition implementation for a {@link AqsReadRemark} serving as the basis of a {@link Lock}
     * implementation.
     *
     * <p>
     * Method documentation for this class describes mechanics, not behavioral specifications from the
     * point of view of Lock and Condition users. Exported versions of this class will in general need
     * to be accompanied by documentation describing condition semantics that rely on those of the
     * associated {@code MyAqs}.
     *
     * <p>
    ...
    public class ConditionObject implements Condition, java.io.Serializable
    {
        ...
        /**
         * Unlinks cancelled waiter nodes from condition queue. Called only while holding lock. This is
         * called when cancellation occurred during condition wait, and upon insertion of a new waiter when
         * lastWaiter is seen to have been cancelled. This method is needed to avoid garbage retention in
         * the absence of signals. So even though it may require a full traversal, it comes into play only
         * when timeouts or cancellations occur in the absence of signals. It traverses all nodes rather
         * than stopping at a particular target to unlink all pointers to garbage nodes without requiring
         * many re-traversals during cancellation storms.
         */
         ...
         * <li>Invoke {@link #release} with saved state as argument, throwing IllegalMonitorStateException
         * if it fails.
        ...
        /*
         * For interruptible waits, we need to track whether to throw InterruptedException, if interrupted
         * while blocked on condition, versus reinterrupt current thread, if interrupted while blocked
        ...
        /**
         * Checks for interrupt, returning THROW_IE if interrupted before signalled, REINTERRUPT if after
         * signalled, or 0 if not interrupted.
         */
         ...
         * @return {@code true} if owned
         */
        final boolean isOwnedBy(AqsReadRemark sync)
        {
            return sync == AqsReadRemark.this;
        }
        
        /**
         * Queries whether any threads are waiting on this condition. Implements
         * {@link AqsReadRemark#hasWaiters(ConditionObject)}.
         *
         * @return {@code true} if there are any waiting threads
         * @throws IllegalMonitorStateException if {@link #isHeldExclusively} returns {@code false}
         */
         ...
         * {@link AqsReadRemark#getWaitQueueLength(ConditionObject)}.
         *
         * @return the estimated number of waiting threads
         * @throws IllegalMonitorStateException if {@link #isHeldExclusively} returns {@code false}
         */
     ...
     * efficient and useful otherwise. So, as the lesser of evils, we natively implement using hotspot
     * intrinsics API. And while we are at it, we do the same for other CASable fields (which could
    ...
    static
    {
        try
        {
            stateOffset = unsafe.objectFieldOffset(AqsReadRemark.class.getDeclaredField("state"));
            headOffset = unsafe.objectFieldOffset(AqsReadRemark.class.getDeclaredField("head"));
            tailOffset = unsafe.objectFieldOffset(AqsReadRemark.class.getDeclaredField("tail"));
